...
for vecIndex, simIDX in enumerate(sm):
    ...
    #with pd.read_csv("https://raw.githubusercontent.com/CIG-UFSCar/DS_Datasets/master/Synthetic/Non-Stationary/RBF1_40k/RBF1_40000.csv",
        ...
        for chunk in reader:
            ...
            fhand.write(f"Summarizing examples from {timestamp} to {timestamp + chunksize-1} -> sim {simIDX} and thrsh {threshIDX}\n")
            ...
            new_row = pd.DataFrame([["["+str(timestamp)+" to "+str(timestamp + chunksize-1)+"]", summarizer.Purity(), summarizer.PartitionCoefficient(), summarizer.PartitionEntropy(), summarizer.XieBeni(), summarizer.ModifiedPartitionCoefficient(), summarizer.FukuyamaSugeno_1(), summarizer.FukuyamaSugeno_2()]], columns=df.columns)
